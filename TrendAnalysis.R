...
#' The following program can be used to apply surprisal analysis to the category data
#' 
#' 
#' Spring 2025, 
#' California
...
#The following script was used to generate Fig 3 in the manuscript
...
ggplot()+
  ...
  labs(x = 'Year', y=TeX("$\\lambda_1$"))->plt.1
...
ggplot()+
  ...
  labs(x = 'Year', y=TeX("$\\lambda_2$"))->plt.2
...
ggplot()+
  geom_line(aes(x = seq(1991, 2024, 1), y=lambdas[,4]), size=1, color= "#D96098")+
  geom_point(aes(x = seq(1991, 2024, 1), y=lambdas[,4]), shape=8, stroke=1, color= "#D96098")+
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.background = element_blank(),
  ...
  labs(x = 'Year', y=TeX("$\\lambda_3$"))->plt.3
